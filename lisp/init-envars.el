;;; init-envars.el --- Load environment variables for project   -*- lexical-binding: t; -*-
...
;; Plz refer to https://humanlytyped.hashnode.dev/working-effectively-with-env-files-in-emacs and https://github.com/cfclrk/setenv-file
;;
...
(defun setenv-file-find-env-file ()
  "Find .env file from current directory all the way up to the project root"
  (let* ((project-root (projectile-project-root))
         (top (if project-root project-root nil))
         (env-file-directory (locate-dominating-file "." setenv-file-dotenv-file-name))
         (file-name (expand-file-name (concat env-file-directory setenv-file-dotenv-file-name))))
    (when (and (file-exists-p file-name)
               (string-prefix-p top file-name))
      file-name)))


(defun setenv-file-set-project-env ()
  "Load all environment variables in the closest .env file."

  (interactive)

  (let ((env-file (setenv-file-find-env-file)) )
    (when env-file
      (let ((str (f-read-text env-file)))
        (setenv-file-str str)))))

(defun setenv-file-unset-project-env ()
  "Unset all environment variables defined in the closest .env file"
  (interactive)
  (let ((env-file (setenv-file-find-env-file)) )
    (when env-file
      (let ((str (f-read-text env-file)))
...
(defun setenv-file-set-envars-hooks ()
  (add-hook 'projectile-mode-hook #'setenv-file-set-project-env)
  (add-hook 'projectile-after-switch-project-hook #'setenv-file-set-project-env)
  (add-hook 'projectile-before-switch-project-hook #'setenv-file-unset-project-env)
  (add-hook 'go-mode-hook #'setenv-file-set-project-env))
